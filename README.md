...
`global.action`                  | Action to perform. [default: `"All"`]<br />`"All"` – Perform a full import from Trello to GitLab. <br />`"AdjustMentions"` – Only adjust mentions in GitLab. <br />`"MoveCustomFields"` – Move the custom fields from Trello card into GitLab description as a list (since GitLab does not yet support custom fields in issues). <br />`"AssocitateWithTrello"` – Post-process GitLab issues and add a comment with a link to the original Trello card. <br />`"DeleteIssues"` – Only delete issues in GitLab.
...
- `--movecustomfields` – Move the custom fields from Trello card into GitLab description as a list (since GitLab does not yet support custom fields in issues).
- `--associatewithtrello` – Post-process GitLab issues and add a comment with a link to the original Trello card.
