...
namespace Trello2GitLab.Conversion.GitLab
{
    ...
    internal class GitLabApi : IDisposable
    {
		...
		/// </remarks>
		public async Task<Upload> UploadFile(string projectId, string filePath)
        {
	        using var content = new MultipartFormDataContent();
	        using var fileContent = new ByteArrayContent(await System.IO.File.ReadAllBytesAsync(filePath));

	        fileContent.Headers.ContentType = new(MimeMapping.MimeUtility.GetMimeMapping(filePath));
	        content.Add(fileContent, @"file", System.IO.Path.GetFileName(filePath));

	        return await Request<Upload>(HttpMethod.Post, $"/projects/{projectId}/uploads", null, content);
        }
